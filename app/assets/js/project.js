...
$.each(db.get('projects_info').filter({ projectId: project_id }).value(), function(key, value) {
    $(".list_task").append('<tr data-toggle="collapse" data-target=".collapse_info_' + value.id + '" class="table-light" data-taskId="' + value.id + '"><th scope="row">' + value.id + '</th><td>' + value.title + '</td><td>' + value.type + '</td><td><div class="progress"><div class="progress-bar" role="progressbar" style="width: ' + value.progress + '%;" aria-valuenow="' + value.progress + '" aria-valuemin="0" aria-valuemax="100">' + value.progress + '%</div></div></td><td style="text-align: center;"><i class="fa fa-pencil-square-o edit-task" aria-hidden="true"></i><i class="fa fa-trash delete-task" aria-hidden="true"></i></td></tr><tr><td colspan="5" class="hiddenRow"><div class="collapse collapse_info_' + value.id + '"><div class="info_task_col">' + value.desc + '</div></div></td></tr>');
});
